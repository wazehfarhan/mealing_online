...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($expense_date)) {
        ...
        // Update expense with updated_at timestamp
        $update_sql = "UPDATE expenses SET amount = ?, category = ?, description = ?, 
                       expense_date = ?, created_by = ?, updated_at = NOW() 
                       WHERE expense_id = ?";
        $update_stmt = mysqli_prepare($conn, $update_sql);
        mysqli_stmt_bind_param($update_stmt, "dsssii", $amount, $category, $description, 
                               $expense_date, $_SESSION['user_id'], $expense_id);
...
// Get category's monthly total
$category_month_sql = "SELECT SUM(amount) as category_month_total 
                       FROM expenses 
                       WHERE category = ? AND expense_date BETWEEN ? AND ?";
$category_month_stmt = mysqli_prepare($conn, $category_month_sql);
mysqli_stmt_bind_param($category_month_stmt, "sss", $expense['category'], $month_start, $month_end);
mysqli_stmt_execute($category_month_stmt);
$category_month_result = mysqli_stmt_get_result($category_month_stmt);
$category_month_total = mysqli_fetch_assoc($category_month_result);
?>
<div class="row">
                    ...
                    <div class="card border-success mb-4">
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-4">
                                    <div class="text-center">
                                        <h6 class="text-muted mb-1">Category Total</h6>
                                        <h5 class="text-success"><?php echo $functions->formatCurrency($category_month_total['category_month_total'] ?: 0); ?></h5>
                                    </div>
                                </div>
                                <div class="col-4">
                                    <div class="text-center">
                            ...
                            <div class="mt-2">
                                <div class="progress" style="height: 10px;">
                                    <?php 
                                    $category_percentage = ($category_month_total['category_month_total'] / $month_total['month_total']) * 100;
                                    if ($category_percentage > 100) $category_percentage = 100;
                                    ?>
                                ...
                                <small class="text-muted">This category represents <?php echo number_format($category_percentage, 1); ?>% of monthly expenses</small>
                            </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <p><strong>Original Category:</strong> <?php echo htmlspecialchars($expense['category']); ?></p>
                    ...
                    <div class="col-md-6">
                        ...
                        $show_updated = isset($expense['updated_at']) && 
                                       !empty($expense['updated_at']) && 
                                       $expense['updated_at'] !== null &&
                                       strtotime($expense['updated_at']) > 0 &&
